...
function PrintPrice(pets) {
  ...
  let count = parseInt(
    prompt("Отличный выбор! Но надо купить корм. Сколько пакетов возьмете: "),
  );
  let WrapDiv = document.getElementById("wrap");
  ...
  if (animal) {
    printPrice.textContent += `За ${count} штук ${animal.food} вы отдадите: ${
      count * animal.food.length * 100
    }\n`;
    printPrice.textContent += `Стоимость ${animal.name} состовляет ${animal.price}\n`;
    printPrice.textContent += `Общая стоимость ${animal.name} состовляет ${
      animal.price + count * animal.food.length * 100
    }\n`;
...
function ChoosePet() {
  ...
  if (likes) {
    ...
    ContinueButton(WrapDiv, function () {
      PrintPrice(CAT);
    });
  } else {
